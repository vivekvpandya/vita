...
int main(int argc, char **argv) {	
	...
	//printf("Rank %d says World Size is %d\n", world_rank,world_size);
	...
	*/
	...
	//	printf("%d is not included in the new reporters comm world\n",world_rank);
	//}
	//printf("%d\n",error);
	...
	if(world_rank == 0){
		...
		MPI_Iprobe(MPI_ANY_SOURCE,0,MPI_COMM_WORLD,&more_to_recv,&status);
		while(more_to_recv){
			
			MPI_Recv(&recv_news,1,mpi_test_type,MPI_ANY_SOURCE,0,MPI_COMM_WORLD,&status);
			cout << "News received successfully!!\n";
	...
	if (rep_rank == 0) {
		...
		for( int n_items = 0; n_items < proc_count ;  n_items++) {
			...
			for ( int r_count = 0; r_count < repo_count; r_count++) {
				printf("Time stamp : %s \n", newsArray[n_items][r_count].timeStamp);
				printf("Title : %s \n", newsArray[n_items][r_count].title);
				printf("Details : %s \n", newsArray[n_items][r_count].details);
			}
		} */
		/*
		int in = 0;
